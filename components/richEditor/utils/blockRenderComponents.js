...
export const Image = props => {
  ...
  const {block, contentState, blockProps: {editor, getEditorState, onChange}} = props;

  // function used to update block data and propagate it to editorState
  const updateBlockData = useCallback((newData = {}, data = block.getData()) => {
  ...
  imgStyle = (imgStyle && data.get("imgStyle").mapKeys(k => camelCase(k)).toJS()) || {};
  figStyle = (figStyle && figStyle.filter(v => v !== "class").mapKeys(k => camelCase(k)).toJS()) || {};
  ...
  return(
    <div ref={container} css={theme => ([figStyle, {position: "relative", zIndex: 1, display: "inline-block", overflow: "hidden", padding: 5, resize, border: `1px dashed ${borderColor(theme)}`}, size.height && size])} onClick={handleClick} onMouseUp={handleMouseUp}>
...
export const StyledBlock = props => {
  const {block} = props;
  const blockStyles = block.getData()
    .filter(v => v !== "class")
    .reduce((styles, v, k) => {
  ...
  if(block.getDepth()) {
    blockStyles.marginLeft = `${block.getDepth() * 2.5}em`;
  }
  return(
...
export const Table = props => {
  const {block, contentState, blockProps: {editor}} = props;
  ...
  if(prevBlock && prevBlock.getType() === "table") {
    ...
    if(target) {
      return(
        createPortal(<EditorBlock {...props}/>, target)
      );
    }
    /**
    ...
    <table css={tableStyle}>
      <tbody>
        {tableShape.map((row, i) => (
          <tr key={i} css={data.get("rowStyle")[i] && Map(data.get("rowStyle")[i]).mapKeys(k => camelCase(k)).toJS()}>
            {row.map((cell, j) => {
